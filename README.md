...
position. Move back to the seam on the peer's side to hand control back, or
press the emergency chord — **Ctrl+Alt+Space** (Windows / **Ctrl+Option+Space**
on macOS) — at any time on either machine to force the release. A refused,
...
the listener searches upward if busy).
